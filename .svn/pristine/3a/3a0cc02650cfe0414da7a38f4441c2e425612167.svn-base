package com.yunchengke.app.ui.view.photoview;
...
public class PhotoView extends ImageView {
    ...
    private final static int ANIMATION_DURATION = 320;
...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    private class Transform implements Runnable {
        ...
        void withTranslate(int startX, int startY, int deltaX, int deltaY) {
            mLastTranslateX = 0;
            mLastTranslateY = 0;
            mTranslateScroller.startScroll(0, 0, deltaX, deltaY, ANIMATION_DURATION);
        }

        void withScale(float form, float to) {
            mScaleScroller.startScroll((int) (form * 10000), 0, (int) ((to - form) * 10000), 0, ANIMATION_DURATION);
        }
        ...
        void withRotate(int fromDegrees, int toDegrees) {
            mRotateScroller.startScroll(fromDegrees, 0, toDegrees - fromDegrees, 0, ANIMATION_DURATION);
        }
    ...
    public void animaFrom(Info info) {
        ...
        if (isInit) {
            ...
            if (info.mWidgetRect.width() < info.mImgRect.width() || info.mWidgetRect.height() < info.mImgRect.height()) {
                ...
                mTranslate.withClip(clipX, clipY, 1 - clipX, 1 - clipY, ANIMATION_DURATION / 3, c);
    ...
    public void animaTo(Info info, Runnable completeCallBack) {
        if (isInit) {
            ...
            mTranslate.withRotate((int) mDegrees, (int) info.mDegrees, ANIMATION_DURATION * 2 / 3);
            ...
            if (info.mWidgetRect.width() < info.mRect.width() || info.mWidgetRect.height() < info.mRect.height()) {
                ...
                postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        mTranslate.withClip(1, 1, -1 + cx, -1 + cy, ANIMATION_DURATION / 2, c);
                    }
