...
import com.yunchengke.app.bean.daemon.DatePickInfo;
import com.yunchengke.app.consts.Constants;
import com.yunchengke.app.utils.daemon.AutoLoadingUtil;
import com.yunchengke.app.utils.daemon.DateTimeUtil;
...
public class DatePickActivity extends BaseActivity {
	...
	LinearLayout linearLayout_date_pick;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		linearLayout_date_pick = (LinearLayout) findViewById(R.id.linearLayout_date_pick);
		/**
		 * 设置加载界面
		 */
		...
		datePickInfo.startDate = DateTimeUtil.getCurrentDateTime();
		datePickInfo.dateRange = 180;
		datePickInfo.selectedDay = DateTimeUtil.getCalendar("2014-2-13");
		
		
		
		new CalendarAsynctask().execute(null, null);
	}
	...
	private LinearLayout getOneLineDayLinearLayout() {
		...
		for (int i = 0; i < 7; i++) {
			float height = (Application.getScreenWidth() - ScreenUtil.dip2px(mContext, 10f)
					- ScreenUtil.dip2px(mContext, 1.5f * 6)) / 7;
			...
			localLayoutParams4.setMargins(ScreenUtil.dip2px(this, 1.5F), ScreenUtil.dip2px(this, 1.5F),
					ScreenUtil.dip2px(this, 1.5F), ScreenUtil.dip2px(this, 1.5F));
			...
			//打包出错改动
			localTextView1.setId(Integer.valueOf(1));
			...
			localTextView2.setTextColor(mContext.getResources().getColor(R.color.calendar_color_white));
...
//					+ "月" + localCalendar.get(Calendar.DAY_OF_MONTH) + "日", Toast.LENGTH_SHORT).show();
	...
	{
		return new ColorStateList(new int[][] { { pressed, enabled }, { selected, enabled }, { enabled }, new int[0] },
				new int[] { -1, -1, mContext.getResources().getColor(R.color.calendar_color_black),
	...
	{
		return new ColorStateList(new int[][] { { pressed, enabled }, { selected, enabled }, { enabled }, new int[0] },
				new int[] { -1, -1, mContext.getResources().getColor(R.color.calendar_color_orange),
						mContext.getResources().getColor(R.color.calendar_color_white) });
	}
	...
	{
		return new ColorStateList(new int[][] { { pressed, enabled }, { selected, enabled }, { enabled }, new int[0] },
				new int[] { -1, -1, mContext.getResources().getColor(R.color.calendar_color_green),
						mContext.getResources().getColor(R.color.calendar_color_white) });
	}
	...
	public LinearLayout drawCanlendar() {
		LinearLayout tempLay = new LinearLayout(mContext);
		tempLay.setOrientation(LinearLayout.VERTICAL);
		...
		Calendar selectedCalendar = (Calendar) datePickInfo.selectedDay.clone();
		int yearOfLocalCalendar1 = localCalendar1.get(Calendar.YEAR);
		int monthOfLocalCalendar1 = localCalendar1.get(Calendar.MONTH);
		Calendar localCalendarEnd = (Calendar) datePickInfo.startDate.clone();
		localCalendarEnd.add(Calendar.DAY_OF_MONTH, datePickInfo.dateRange - 1);
		...
		for (int i = 0; i < totalDiffer; i++) {
			LinearLayout item_date_pick = (LinearLayout) View.inflate(mContext, R.layout.item_date_pick, null);
			tempLay.addView(item_date_pick);

			TextView tv_cal_year = (TextView) item_date_pick.findViewById(R.id.tv_cal_year);
			TextView tv_cal_month = (TextView) item_date_pick.findViewById(R.id.tv_cal_month);
			...
			for (int j = 0; j < lines; j++) {
				...
				if (j == 0) {// 第一行
					for (int k = 0; k < 7; k++) {
						TextView localTextView = (TextView) (((RelativeLayout) oneLineLinearLayout.getChildAt(k))
								.getChildAt(0));
					...
					for (int k = 0; k < 7; k++) {
						...
						Calendar tempCalendar3 = (Calendar) tempCalendar2.clone();
						tempCalendar3.set(Calendar.DAY_OF_MONTH, index);
						String date = tempCalendar3.get(Calendar.YEAR) + "-" + (tempCalendar3.get(Calendar.MONTH) + 1)
								+ "-" + tempCalendar3.get(Calendar.DAY_OF_MONTH);
						localTextView.setTag(Long.valueOf(tempCalendar3.getTimeInMillis()));
						localSelectedRela.setTag(Long.valueOf(tempCalendar3.getTimeInMillis()));
						if (compareCal(tempCalendar3, calendarToday) == -1) {// 小于当天
	...
	class CalendarAsynctask extends AsyncTask<Void, Void, LinearLayout> {

		@Override
		protected LinearLayout doInBackground(Void... params) {
			// TODO Auto-generated method stub
			
			LinearLayout layout = drawCanlendar();
		...
		protected void onPostExecute(LinearLayout result) {
			...
			linearLayout_date_pick.addView(result);
		}
	}
