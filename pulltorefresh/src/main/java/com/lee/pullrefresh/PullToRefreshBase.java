...
import com.lee.pullrefresh.ILoadingLayout.State;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	private void init(Context context, AttributeSet attrs) {
		...
		disableScrollMode(mRefreshableView);
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		if (!isPullLoadEnabled() && !isPullRefreshEnabled() && !isScrollLoadEnabled()) {
			return false;
		}
				...
				if (!isPullDown() && !isPullUp() ) {
					...
					if (deltaY > 0) { // 下拉
						...
						if (isPullRefreshEnabled() && isReadyForPullDown()) {
							LogUtil.e("onInterceptTouchEvent", "isReadyForPullDown");
							...
							mIsHandledTouchEvent = (Math.abs(getScrollYValue()) > 0 || deltaY > 1f);
						...
						LogUtil.e("onInterceptTouchEvent", "上拉");

						if ( absDiff > mTouchSlop && isPullLoadEnabled() && isReadyForPullUp()) {
							LogUtil.e("onInterceptTouchEvent", "isPullLoadEnabled");
						...
						} else if ( absDiff > mTouchSlop && isScrollLoadEnabled() && isReadyForPullUp()) {
							LogUtil.e("onInterceptTouchEvent", "isScrollLoadEnabled");
					...
					LogUtil.e("onInterceptTouchEvent", "刷新中");
					...
					} else if ( absDiff > mTouchSlop &&  isPullLoadEnabled() && isReadyForPullUp()) {
						LogUtil.e("onInterceptTouchEvent", "isPullLoadEnabled+");
...
//			if (!isPullRefreshing() && !isPullLoading()) {
			...
			if (!isPullDown() && !isPullUp()) {
				...
				if (deltaY > 1f) {
					...
					if (isPullRefreshEnabled() && isReadyForPullDown()) {
						...
						LogUtil.e("onTouchEvent", "isPullRefreshEnabled");
					...
					if (isScrollLoadEnabled() && isReadyForPullUp()) {

						LogUtil.e("onTouchEvent", "isPullRefreshEnabled");

						handled = doLoading(deltaY);
					} else if (isPullLoadEnabled() && isReadyForPullUp()) {

						LogUtil.e("onTouchEvent", "isPullLoadEnabled");
				...
				if (isPullRefreshEnabled() && isReadyForPullDown()) {
					...
					pullHeaderLayout(deltaY / OFFSET_RADIO);
					handled = true;
				} else if (isScrollLoadEnabled() && isReadyForPullUp()) {

					LogUtil.e("onTouchEvent", "isScrollLoadEnabled+");
		...
		case MotionEvent.ACTION_UP:
			if (mIsHandledTouchEvent) {
				...
				if (isReadyForPullDown()) {
					...
					// 加载更多
					if ((isPullLoadEnabled() || isScrollLoadEnabled())
    ...
    public boolean doLoading(float deltaY) {
	...
	protected boolean isPullDown() {
		return (mPullDownState == State.PULL_TO_REFRESH || mPullDownState == State.REFRESHING
				|| mPullDownState == State.RELEASE_TO_REFRESH);

	}

	protected boolean isPullUp() {
		return (mPullUpState == State.PULL_TO_REFRESH || mPullUpState == State.REFRESHING
				|| mPullUpState == State.RELEASE_TO_REFRESH);
	...
	/**
	 * 关闭下拉悬停等OverScroll效果
	 * API8以上调用setOverScrollMode，API8及以下通过反射
	 * @param view
	 */
	...
	public static void disableScrollMode(View view) {
		if (Build.VERSION.SDK_INT >= 9) {
			...
			try {
				...
				int OVER_SCROLL_NEVER = (Integer) viewCls.
						getField("OVER_SCROLL_NEVER").get(view);
